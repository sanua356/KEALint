use crate::{
    common::RuleResult,
    configs::{hooks::KEAHookLibrary, hosts_database::KEAHostsDatabase},
    constants::HOST_CMDS_HOOK_LIBRARY,
};

pub fn get_no_activated_host_cmds_for_database_backend_rule(
    ...
    let is_found_host_cmds_hook = hooks_libraries
        .as_ref()
        .unwrap_or(&vec![])
        .iter()
        .any(|hook| hook.library.contains(HOST_CMDS_HOOK_LIBRARY));

    let is_found_hosts_database = hosts_database.is_some();
    let is_found_hosts_databases = match hosts_databases {
...
mod tests {
    ...
    fn check_expected_trigger() {
        ...
        let rule = get_no_activated_host_cmds_for_database_backend_rule(
