...
impl FromStr for KEAv4PoolVariant {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        if IPV4_RANGE_REGEXP.is_match(s) {
            ...
            let start_ip: Ipv4Addr = start.parse().map_err(|_| "Bad start ip in range.")?;
            let end_ip: Ipv4Addr = end.parse().map_err(|_| "Bad start ip in range.")?;
        ...
        if CIDR_V4_REGEXP.is_match(s) {
            ...
            let start_ip: Ipv4Addr = start.parse().map_err(|_| "Bad start ip in range.")?;
            let prefix_value: u8 = prefix.parse().map_err(|_| "Bad prefix in CIDR.")?;
